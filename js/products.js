...
function importProductsFromCollection(collection, container) {
	...
	getProducts(collectedProducts).then((products) => {
        ...
        <div class="product_card">
          <h1 class="product_stock">x${totalInventory}</h1>
      ...
      `;
			...
			if (totalInventory <= 5) {
				productElement
					...
					.classList.add('low_stock');
			}

			container.appendChild(productElement);

			productElement
				.querySelector('.product_card')
				.addEventListener('click', async function () {
					if (!product_add_panel.classList.contains('open')) {
						...
						product_add_panel.querySelector('.cart_item_title').innerHTML =
							productTitle;
...
product_add_panel
	.querySelector('.increment_button')
	.addEventListener('click', function () {
...
product_add_panel
	.querySelector('.decrement_button')
	.addEventListener('click', function () {
		const id = product_add_panel.dataset.id;
		decrement(id);
		update(id);
